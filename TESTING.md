...
> **v5.0.0 でのコマンド統合**: 登録コマンドを **20本 → 9本**に束ね、**すべて接頭辞「匠」+ 漢語**にした
> （匠戦略／匠調査／匠顧客／匠発信／匠広報／匠危機対応／匠計測／匠設定／匠検証）。
> **手順書は1本も消えていない**（旧20本の中身はそのまま残り、玄関の手順書から振り分けられる。
> 対応表は [docs/command-registry.md](docs/command-registry.md) の「旧コマンド名からの対応」）。
> **本ファイルの v4.x 以前のログは当時のコマンド名のまま残す**（履歴の改竄を避けるため）。
> 到達性は lint #28 と `tests/test_channel_packs.py` が、命名規約は lint #29 が機械で守る。
