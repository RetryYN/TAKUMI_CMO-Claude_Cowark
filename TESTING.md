...
| # | 内容 |
|---|---|
| **O-1** | **RM Guard がヒアドキュメント本文中の文字列に反応する**。削除操作を一切していないレポート生成スクリプト（`cat > x.py <<'EOF' … EOF`）が、本文の説明文に再帰削除フラグの表記を**文字列として**含んでいたために deny された。フェイルクローズ側の誤爆なので安全性は損なわれていないが、「削除について書く」作業が止まる |
