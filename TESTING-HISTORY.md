# 検証の履歴ログ（第1〜8ラン / v0.8.0〜v2.0.0）

> **これは過去ログである。現在の検証手順・最新ランの結果は [TESTING.md](TESTING.md) が正本。**
> ここは追記された順に残す — **当時の名称・当時の判断のまま**書き換えない（履歴の改竄を避ける）。
> 旧コマンド名（`/検証`・`/セットアップ` 等）や旧プラグイン名が出てくるのは意図的で、
> 対応表は [docs/command-registry.md](docs/command-registry.md) の「旧コマンド名からの対応」。
>
> **なぜ分けたか（2026-07-26）**: TESTING.md が 694 行あり、うち約8割がこの履歴だった。
> 「いま何が検証済みで何が未実施か」を知るために毎回スクロールする状態は、
> **正本が読まれなくなる**という一番よくない壊れ方に近い。読む頻度で切った。
