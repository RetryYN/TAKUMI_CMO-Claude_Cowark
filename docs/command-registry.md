...
| growth-teardown-jp | **リサーチ**（/リサーチ）・コンテンツ・SNS運用・戦略 | 伸びているアカウント/サイトの分解 — 一次観測の項目、生存者バイアス回避（伸びていない同業を必ず入れる）、**実測した5件の分解**（サイボウズ式/SmartHR Mag./LIGブログ/mercan/北欧、暮らしの道具店）、観測→仮説への変換。実行手順は `docs/parts/growth-teardown.md` |
