...
| **技** | ユーザーにも AI にも届く形に磨き上げ、伝わることを実現する | `references/seo-jp`（検索意図）/ `cro-jp`（転換率）/ `design-evidence-jp`（可読性・コントラスト等の実証基準）/ `web-design` / 執筆スキル群 ＋ **design-artisan → design-critic の審査ループ**（Critic Gate が PASS 前の引き渡しを機械遮断） |
...
> 未実装（次フェーズ候補）: 「技」の *AI に届く* 側 — AI 検索での言及・引用の観測と最適化。現状は seo-jp とオーガニック露出の厚みで間接的に効かせているだけで、測る仕組みはない。README のコンセプトでは市場認識として書き、機能としては約束していない。
...
| **サブエージェント** | 専門作業の職人。タスクから呼ばれる | `agents/`（7体: writer / artisan / critic / advisor / pre-send-verifier / outcome-verifier / **cmo-strategist**（上流ループの立案）） |
