...
| ad-compliance-jp | コンテンツ・全公開物 | 日本の表示規制チェック（オーガニック含む全公開物） — **§0 適用判定**で効く法域を絞り、景表法／ステマ規制／薬機法／**健康増進法**（食品・「何人も」が対象）／**特商法 通信販売**（表示事項・メール広告オプトイン・返品特約・最終確認画面）／職業安定法／**医療広告規制（AI は判断せず人間に返す）**を検査。**法規で許されてもプラットフォーム規約で禁止されていることがある** |
