...
| 層 | 概念 | 実体 |
|---|---|---|
| **コマンド** | カテゴリーレベル（媒体・対象のタスクパック）。要望は引数に自由に書かせる | `commands/`（登録16本・日本語名）+ `procedures/takumi-*.md`（内部手順含め33本） |
...
1. **登録コマンド（`commands/`）は日本語名が本体** — メニューに並ぶのはこの16本のみ。description には自動発火用の「Use when」を日本語で書く
...
**計: 登録コマンド 16 / 内部手順 17 / 手順書 33（procedures/）**
