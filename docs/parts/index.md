...
**委譲時のパス規約**: サブエージェントに references/ や templates/ を参照させるときは、メインループが `${CLAUDE_PLUGIN_ROOT}` を解決した**実絶対パス**を依頼プロンプトに明記して渡す（サブエージェントの cwd はワークスペースで、相対・cwd起点Globではプラグイン実体に届かない — 2026-07-24 検証で「不在」誤申告が3件発生した根本原因）。
