...
1. **計測データ JSON**: knowledge/data/takumi.db の audit_pages（DBが無ければ templates/db-schema.sql で初期化。detail_json に全計測値）
...
- 読み取り専用タスク（navigate / evaluate / network_requests / screenshot のみ）。匠ゲート対象外
