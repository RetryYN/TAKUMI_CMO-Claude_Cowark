...
**`model: opus` を明示して同じプロンプトで再委譲する**（自動フォールバックは無い＝呼び出し側の責務）。

> **fable の代替に `sonnet` を置かない（2026-07-26 訂正）。** 直前の段落が
> 「**fable の段を下げると成果物の質が直接落ちる**」と書いているのに、
> **その4行後で速い側へ降ろしていた** — 正本が自分の中で矛盾していた。
> `risk-forecaster` 側には最初から「**判断を担う役なので sonnet への降格はしない**」と書いてあり、
> **同じ規則が design-artisan にだけ適用されていなかった。**
>
> **規則（全 fable エージェント共通）: fable が使えないときは、その級の「深く考える側」へ横移動する。速い側へ降ろさない。**
> 戦略級・戦術級・職人級とも深く考える側は `opus`。`scripts/lint.py` #54 が
> 「fable の代替に sonnet を書いていないか」を機械検査する。
...
## 級はどこで使われるか — タスク単位のループ

級は分類のためではなく、**1つの仕事を回すための配役**である。
タスク1件は4フェーズで回り、**フェーズと級は1対1**（正本は [parts/task-loop.md](parts/task-loop.md)）:

**① 構える（戦略級）→ ② 作る（職人級）→ ③ 咎める（戦術級）→ ④ 確かめる（作業者級）**

③で差し戻しなら②へ戻り、**最大2周**。3周目に入らず人間へ返す。
`takumi/domain/task_loop.py` が「完了条件なしで始めない／級を取り違えない／
同じフェーズを続けて回さない／3周目に入らない／確かめずに完了しない」を機械で止める。

**並列になるのは①だけ**（合議2体・壁打ち2体）。②③④は直列 —
作る側を並列にしても審査が1本なら詰まる。
...
3. **本プラグインの並列は元々2〜3体** — `docs/parts/pre-setup-council.md` の合議は常設2体、
   `/匠戦略` の壁打ちは2体。**4体あれば設計上の並列はすべて収まる**
