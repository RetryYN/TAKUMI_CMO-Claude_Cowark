...
> ✅ **v2.0.2 — 匠CMO 到達**。堅牢に検証された「Browser Worker for Claude Cowork」基盤を土台に、
...
- **永続フォルダを接続してください**: 未接続だと、専門エージェントがプラグイン同梱の執筆規範（`references/`）に到達できず、
  デザイン審査や法規チェックが規範に基づかなくなります（escalations E6）。[継続利用する場合](#継続利用する場合推奨)を参照。
- **既知の限界（多層防御の前提）**:
  - **安全ゲートが機械強制されるのは Cowork 実機のみ。ローカルでの hook 配線は環境により振れます**（2026-07-24 は未配線／2026-07-25 は配線ありを実測。escalations E4）。配線されていなければ全ゲートがフェイルオープンになります。さらに Cowork のバージョン・構成によって plugin hooks が発火しない上流報告もあるため、**hooks を唯一の防御にせず、自己規律＋人間承認と併用**します。ゲート依存の運用（一括送出・金銭近傍・無人運用・ブランド分離）は cloud で行い、Tier 2 検証で実際の発火を確認してください。
