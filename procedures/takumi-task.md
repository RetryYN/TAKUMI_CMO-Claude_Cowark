...
### 2. タスクYAML案の生成（**§5 の壁打ちを通すまでは「案」**）

> **玄関（`/匠設定 ▸ takumi-customize`）を経ずに直接ここへ入ったときは、先に「①構える」を確かめる** —
> このタスクは**どの目的・どのKPIに紐づくのか**。答えられないまま YAML を書くと、
> **手順だけが先に固まって目的が後追いになる**（2026-07-27 第15ラン V81 の所見）。
> 目的が無いなら `/匠戦略` へ戻す。**YAML は §5 で strategy-advisor の審査を通すまで確定させない。**
