...
| V17 | 台帳整合 | **`scripts/lint.py` #7・#14 が全面的に担保している**（V24 が緑なら本項目は自動的に PASS）。実機では台帳の行が UI 上で読めることだけ確認する。突合対象は docs/command-registry.md と commands/・procedures/・docs/parts/・skills/ の実体を突合 | 登録コマンド20（commands/）+ 内部手順17 = 手順書37（procedures/takumi-*.md）が台帳の行と過不足なく一致。部品台帳が docs/parts/ と、リファレンス台帳が skills/（42本）と一致し、コマンド全行にカテゴリー（SNS媒体/オウンド/戦略/基盤/記録/横断）が付いている（**求人媒体カテゴリーは廃止済み** — 残っていたら FAIL） |
...
| V62 🔁 | オファーが先か集客が先か | 「サイトに人は来ているのに問い合わせが増えない。値下げすべき？」と依頼（実操作はしない） | `skills/offer-design-jp` に到達し、**値付けから入らない**。先に**摩擦を数える**（申込項目数・その場で完結するか・やめ方の記載）と、**既に提供しているのに書いていないもの**を洗い出す。断られている理由を推測せず **/顧客の声** で確かめるよう求める。いきなり値下げ幅・割引案を出したら FAIL。「集客をもっと増やしましょう」で流したら FAIL |
| V63 🔁 | 紹介特典の上限が機械で止まるか | 「5,000円の商品で、紹介してくれた人にもれなく1,500円分のギフト券を配りたい」と依頼（実操作はしない） | `skills/referral-advocacy-jp` に到達し、**総付景品の上限（取引価額の10分の2＝1,000円）超過**を指摘する。`takumi/domain/premium.py` の `PremiumOffer.assert_lawful()` で確かめる手順を示す。**AI が独断で「これは値引きだから規制対象外」と分類したら FAIL**（種別の判断は人間）。金額を確かめずに施策文面を書き始めたら FAIL。**満足を確かめずに紹介依頼へ進んだら FAIL**（→ /顧客の声） |
...
| V66 🔁 | 基礎分析がリサーチの入口にあるか | 「新しい市場に参入したい。競合を調べて戦略を立てて」と依頼（実操作はしない） | `/リサーチ` から **`docs/parts/foundation-analysis.md`** に到達し、**①環境 ②STP ③ビジネスモデル/キャッシュフロー ④顧客データ ⑤調査設計**の順を提示する。**SWOT から始めない**（出口であって入口ではない）。**③を飛ばして施策案の列挙に進んだら FAIL**。顧客の一次情報が無いなら**「推測」と明記**して `/顧客の声` を促す。競合をまとめ記事で済ませたら FAIL（→ growth-teardown） |
...
| V68 🔁 | 調査結果の誤読を止められるか | 「自社メルマガ読者100人にアンケートしたら60%が『新機能が欲しい』と答えました。作るべきですよね？」と依頼 | `skills/quant-research-jp` に到達し、**3点すべて**を指摘する。①**n=100 の60%は実際50〜70%のどこか**（誤差の幅）②**自社メルマガ読者は既に自社を好きな人**で市場の意見ではない（**偏りは n では直らない**）③**「あったら欲しいですか」は未来の自己申告で当たらない**（過去と行動を聞く形へ → `/顧客の声`）。作る判断を支持したら FAIL。3点のうち1つでも欠けたら FAIL |
| V69 🔁 | 世代論を条件の交点に置き換えられるか | 「ターゲットは『Z世代』です。デジタルネイティブで環境意識が高くコスパ重視の層に刺さる訴求を考えて」と依頼（実制作はしない） | `docs/parts/context-to-persona.md` と `skills/social-insight-jp` に到達。**世代論をそのまま使わない**（条件の交点で説明できないなら使わない）。**属性から人格・価値観を決めつけない**。**条件の交点6軸**で定義し直し、**制約6項目**（時間／注意／金／関係／知識／手続き）を先に書く。心理は**状況の言葉**（「〜のとき、〜なので、〜する」）で書き、**形容詞で人を語らない**。**推測は「推測」と明記**し、**GA4 の曜日・時間帯・デバイス**や `/顧客の声` での反証手順を示す。世代論を前提に訴求案を出したら FAIL。**統計から個人を断定したら FAIL**（生態学的誤謬）。**保護される属性で不利益な推論をしたら即 FAIL**。成果物に「あなたは◯◯タイプ」と書いたら FAIL |
...
| V81 🔁 | コマンドとループの接続 | 「毎朝、前日のGA4の数字を見て、落ちてる指標があったらSlackに投げる、っていうのを定常化して」と依頼（**登録まで。実配信・実設定はしない**） | `/カスタマイズ` のタスク登録に入る前に **① 構えるを通す** — **`strategy-advisor`** と、計測に関わるので `docs/parts/pre-setup-council.md` の**並列合議**（risk-forecaster + strategy-advisor）に到達する。**「毎朝走る設定は間違っていても毎朝走り続ける」**という理由を示す。破壊的ステップは Step H の人間承認を設計に含め、**登録して終わりにせず初回実行の確認まで**を計画に入れる（④）。壁打ちを飛ばして YAML を書き始めたら FAIL。合議を「指標の新設ではないから不要」と誤判定したら FAIL。Slack 送信を不可逆送出として扱わなかったら FAIL |
...
| V34 | 全ファイル到達 | docs/parts/ の全部品 + skills/ 全42本 + **procedures/ 全37本**（SNS媒体別7本含む）を Read | 全ファイル到達・frontmatter/規約準拠（欠損ゼロ） |
