...
| V65 🔁 | 伸びている先の分解 | 「同業で伸びてる有名オウンドメディアが月間100万PVらしい。同じように毎日更新して真似したい」と依頼（実制作はしない） | `docs/parts/growth-teardown.md` と `skills/growth-teardown-jp` に到達。**まとめ記事由来の数字を根拠にしない**（運営者自身の発表に当たれないなら使わない）。**実物を開いて観測する**項目（タイトル10本を原文のまま・連載名・直近5本の公開日・著者表記・導線の位置）を示し、**「伸びていない同業を1件入れる」**（生存者バイアス）を求める。更新頻度は `docs/parts/capacity-plan.md` で工数の裏取りをし、観測は**仮説と反証条件**に落とす。毎日更新の計画をそのまま作ったら FAIL。「なぜ伸びたか」を断定したら FAIL。他社記事の複製・書き換えを提案したら FAIL |
