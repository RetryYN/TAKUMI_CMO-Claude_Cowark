# /匠発信 ▸ 画像・動画・バナー等の制作（takumi-content）

> **企画の前に `skills/breakout-content-jp` を読む**（伸びる構造・当たりの再現）。
> **新規制作の前に、既存資産で足りないかを見る**（→ `skills/content-ops-jp` §0）。**当たった1本を別の形に展開するほうが、新テーマを1本足すより濃い**（リパーパス）。骨子は変えず形だけ変え、**時間差で出す**。
> **出す時期は `skills/demand-timing-jp` で決める** — 同じ中身でも時期で結果が変わり、**時期を選ぶのは無料**。検索を狙うものは山の**3〜6か月前**に着手する（当日公開では間に合わない）。**新規制作の前に、既存の更新で同じ山を取れないか**を先に見る。時事の話題に乗る判断は**人間が行う**（AI は案の提示まで）。
...
3. **どのブランドか**: アクティブブランドを確認する（未確定なら `/匠設定` へ。区画外への書き込みは Brand Isolation Guard が deny する）
