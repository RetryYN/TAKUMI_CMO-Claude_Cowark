...
6. **法規の適用判定を一度だけ決めて記録する**（→ `skills/ad-compliance-jp` §0）。`brand.yaml` に「このブランドに効く法域」（景表法・ステマ規制は全ブランド／薬機法・**健康増進法**・**特商法 通信販売**・職業安定法・**医療広告規制**は事業内容による）を書く。**毎回の推測をやめるための記録**であり、書いていないと公開前チェックが節ごとに揺れる。**医療機関・士業・金融は AI が可否を判断しない領域**なのでここで明示しておく。あわせて、**商圏のある事業（実店舗・訪問・来訪）なら `skills/local-seo-jp` の対象**である旨も記録する（該当しないブランドにローカル施策を持ち込まないため）。
